...
export default function Auth({ onBack }) {
  ...
  const handleAuth = async (e) => {
    ...
    if (import.meta.env.VITE_SUPABASE_URL === 'your_supabase_url_here' || !import.meta.env.VITE_SUPABASE_URL) {
      setMessage('Error: Supabase keys not configured in frontend/.env');
      setLoading(false);
      return;
    }
    ...
    try {
      if (mode === 'signup') {
        ...
        if (error) throw error;
        // Successful login will be caught by App.jsx auth listener
      }
    } catch (error) {
      ...
      <div style={{ width: '100%', maxWidth: 420 }}>
        ...
        <div style={{
          ...
          <form onSubmit={handleAuth} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            {message && (
              <div style={{ padding: 12, borderRadius: 8, background: message.includes('Error') ? '#FEE2E2' : '#ECFDF5', color: message.includes('Error') ? '#DC2626' : '#059669', fontSize: 13, fontWeight: 500, lineHeight: 1.5 }}>
                {message}
              </div>
            )}
